...
export function MyCoursesView({
  ...
  // Filter courses where current person is enrolled
  ...
  const getNextSession = (courseId: number) => {
    ...
    return courseSessions
      ...
      .sort((a, b) => new Date(a.date_init).getTime() - new Date(b.date_init).getTime())[0]
  }
              ...
              <Card key={course.id} className="flex flex-col">
                ...
                <CardContent className="flex-1 space-y-4">
                  <div className="flex items-center gap-2 text-sm text-muted-foreground">
                    ...
                    <span>{place?.name || 'Sin lugar'}</span>
                    {place?.busy && (
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="h-2 overflow-hidden rounded-full bg-muted">
                      <div
                        ...
                        style={{
                          width: `${courseSessions.length > 0
                              ? (completedSessions / courseSessions.length) * 100
                              : 0
                            }%`,
                        }}
                      />
                  ...
                  {nextSession && (
                    <div className="rounded-lg border bg-muted/50 p-3">
                      ...
                      <p className="mt-1 text-sm text-muted-foreground">
                        {new Date(nextSession.date_init).toLocaleDateString('es-ES', {
                          weekday: 'long',
                          day: 'numeric',
                          month: 'long',
                        })}
                      ...
                      <p className="text-sm text-muted-foreground">
                        {new Date(nextSession.date_init).toLocaleTimeString('es-ES', {
                          hour: '2-digit',
                          minute: '2-digit',
                        })}{' '}
                        -{' '}
                        {new Date(nextSession.date_end).toLocaleTimeString('es-ES', {
                          hour: '2-digit',
                          minute: '2-digit',
                        })}
                      </p>
                      <Badge className={`mt-2 ${getStatusColor(nextSession.status)}`}>
                        {nextSession.status}
                      </Badge>
                    </div>
                  )}
